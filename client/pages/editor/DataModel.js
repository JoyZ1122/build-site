 ...
 */
let getCommonStyle = function (styleObj, scalingRatio = 1, name = '') {
  let needUnitStr = ['width', 'height', 'top', 'bottom', 'left', 'paddingTop', 'paddingLeft', 'paddingRight', 'paddingBottom', 'marginTop', 'marginLeft', 'marginRight', 'marginBottom', 'borderWidth', 'fontSize', 'borderRadius', 'letterSpacing']
  let style = {}
  for (let key in styleObj) {
    if (needUnitStr.includes(key)) {
      if (name == 'qk-popbutton' && key == 'height') {
        style[key] = (styleObj[key] * 1) + 'px'
      } else {
